...
export default function PlantsSection() {
  ...
  return (
    <section
      id="plants"
      className="bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 py-36"
    >
        ...
        <div className="mx-auto max-w-6xl">
          ...
          >
            <div className="flex justify-center mb-8">
              <TabsList className="bg-gray-100 dark:bg-gray-800 p-2 rounded-xl flex-wrap justify-center gap-2">
                {plantCategories.map((category) => (
                  <TabsTrigger
                    key={category.id}
                    value={category.id}
                    className="flex items-center gap-2 whitespace-nowrap px-4 py-2.5 text-sm font-medium data-[state=active]:bg-white dark:data-[state=active]:bg-gray-700 data-[state=active]:text-red-600 dark:data-[state=active]:text-red-400 rounded-lg transition-all shadow-sm data-[state=active]:shadow-md"
                  >
                    {category.icon}
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {displayedImages.map((image) => (
                  <Card
                    key={image.id}
                    className="flex flex-col overflow-hidden border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm hover:shadow-md transition-all duration-300"
                  >
                    <div
                      onClick={() => handleSelectPlant(image)}
                      className="cursor-pointer w-full text-left flex flex-col flex-grow"
                    >
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-5xl p-0 bg-white dark:bg-gray-800 rounded-lg flex flex-col max-h-[90vh]">
          {selectedPlant && (
            <>
